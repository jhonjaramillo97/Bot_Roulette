...
- `_send_crash_report_telegram()` --calls--> `load_credentials()`  [EXTRACTED]
  bot_ruleta/debug_logger.py → bot_ruleta/config.py
- `_update_tunnel_url()` --calls--> `load_credentials()`  [EXTRACTED]
  bot_ruleta/launcher.py → bot_ruleta/config.py
- `main()` --calls--> `load_credentials()`  [EXTRACTED]
  bot_ruleta/scan_lobby_ids.py → bot_ruleta/config.py
- `run_bot()` --calls--> `load_credentials()`  [EXTRACTED]
  bot_ruleta/scanner.py → bot_ruleta/config.py
...
Cohesion: 0.07
Nodes (57): guardar_resultado(), init_db(), Guarda un resultado en la tabla específica del juego., Inicializa la base de datos creando las tablas configuradas., capture_screenshot(), _cleanup_old_crash_reports(), _cleanup_old_screenshots(), generate_crash_report() (+49 more)

### Community 1 - "Community 1"
Cohesion: 0.08
Nodes (37): get_color_streak_threshold(), get_number_delay_threshold(), load_credentials(), Configuración centralizada del bot de ruleta. Constantes, configuración de mesas, Lee el umbral de racha de color. Prioridad: runtime overrides > GUI saved > .env, Lee el umbral de retraso de números individuales. Prioridad: runtime overrides >, Lee credenciales. Prioridad: runtime overrides > .env, check_and_notify() (+29 more)
...
Cohesion: 0.15
Nodes (17): get_connection(), limpiar_mesa(), obtener_ultimo_numero(), obtener_ultimos_numeros(), Manejo de base de datos SQLite con tablas separadas por juego., Obtiene el último número registrado en la tabla del juego., Obtiene los últimos N registros (número, color y timestamp)., Resuelve el nombre de tabla SQLite a partir del nombre descriptivo. (+9 more)
