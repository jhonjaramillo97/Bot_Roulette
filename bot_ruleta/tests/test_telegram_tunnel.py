...
class TestSendTelegramMsg:
    @patch("bot_ruleta.telegram.requests.post")
...
class TestTunnel:
    def test_dev_mode_true_returns_none(self, monkeypatch):
        monkeypatch.setattr("bot_ruleta.tunnel.DEV_MODE", True)
        from bot_ruleta.tunnel import get_cf_env_vars
        token, domain = get_cf_env_vars()
        assert token is None
        assert domain is None

    def test_prod_mode_returns_token(self, monkeypatch):
        monkeypatch.setattr("bot_ruleta.tunnel.DEV_MODE", False)
        from bot_ruleta.tunnel import get_cf_env_vars
        token, domain = get_cf_env_vars()
    ...
    def test_notify_tunnel_url_does_not_crash(self, monkeypatch):
        monkeypatch.setattr("bot_ruleta.telegram.send_telegram_msg", lambda *a, **kw: False)
