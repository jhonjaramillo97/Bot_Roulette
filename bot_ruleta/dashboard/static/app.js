...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (btnFilter) {
        btnFilter.addEventListener("click", () => {
            ...
            // Toggle modern CSS class
            btnFilter.classList.toggle("active", filterSignalsOnly);
...
// ── TOGGLE ACCORDION: Función global llamada directamente desde onclick inline ──
function toggleAccordion(headerEl) {
    const card = headerEl.closest(".table-card");
    if (!card) return;
    ...
    if (gridEl.classList.contains("list-view")) {
        ...
        if (tableName) {
            window.location.href = '/mesa?mesa=' + tableName;
        }
    }
}
...
function renderGrid(tables) {
    ...
    // Solo reconstruir si cambió el número de mesas (primera vez o si se agregan/quitan)
    // Después solo actualizamos los valores para evitar flickering y pérdida de estado
    const existingCards = grid.querySelectorAll(".table-card[data-table]");
    const tableNames = tables.map(t => t.table_name).sort().join(',');
    const existingNames = Array.from(existingCards).map(c => c.dataset.table).sort().join(',');
    
    if (tableNames !== existingNames) {
...
function buildCards(grid, tables) {
    // Preservar qué mesas estaban expandidas antes de reconstruir el DOM
    const expandedTables = new Set();
    grid.querySelectorAll(".table-card.expanded").forEach(c => {
        if (c.dataset.table) expandedTables.add(c.dataset.table);
    });
    ...
    tables.forEach(t => {
        ...
        // Marcar si esta tarjeta estaba expandida antes de la reconstrucción
        card._wasExpanded = expandedTables.has(t.table_name);
        ...
        // Si la tarjeta estaba expandida, forzar el body visible con inline style
        if (card._wasExpanded) {
            const body = card.querySelector(".accordion-body");
