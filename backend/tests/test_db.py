...
class TestValidateTableName:
    def test_valid_table(self):
        from backend.database.db import validate_table_name
...
class TestDbCRUD:
    ...
    def test_save_result_and_get_last_numbers(self):
        from backend.database.db import save_result, get_last_numbers
        table = TABLES[0]
    ...
    def test_get_last_number(self):
        from backend.database.db import save_result, get_last_number
        table = TABLES[0]
    ...
    def test_get_last_number_empty_table(self):
        from backend.database.db import get_last_number
        n = get_last_number(TABLES[0]["name"])
        assert n is None

    def test_clear_table(self):
        from backend.database.db import save_result, get_last_numbers, clear_table
        table = TABLES[0]
