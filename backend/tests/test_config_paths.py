...
class TestConfig:
    ...
    def test_reds_count(self):
        from backend.config.config import REDS
        assert len(REDS) == 18
    ...
    def test_lobby_mode_is_bool(self):
        from backend.config.config import LOBBY_MODE
        assert isinstance(LOBBY_MODE, bool)

    def test_afk_interval_positive(self):
        from backend.config.config import AFK_INTERVAL
...
class TestPaths:
    def test_is_frozen_false_in_dev(self):
        from backend.config.paths import is_frozen
        assert is_frozen() is False

    def test_get_base_dir_returns_valid_path(self):
        from backend.config.paths import get_base_dir
        path = get_base_dir()
